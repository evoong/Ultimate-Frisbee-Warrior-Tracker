...
export default function ChemistryHub({ pairs, loading, error, emptyLabel }: {
  ...
  // See PerformanceChart: the list stays and dims while the next range
  // loads, and the skeleton is only for a panel with nothing in it yet.
  const cold = loading && pairs.length === 0
  const busy = loading && pairs.length > 0
