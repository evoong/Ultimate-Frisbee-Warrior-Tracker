...
app.get('/api/seasons/meta', async (req, res) => {
  try {
    const organizers = await pool.query('SELECT DISTINCT organizer FROM seasons WHERE organizer IS NOT NULL ORDER BY organizer')
    const names = await pool.query('SELECT DISTINCT name FROM seasons WHERE name IS NOT NULL ORDER BY name')
    const years = await pool.query('SELECT DISTINCT year FROM seasons WHERE year IS NOT NULL ORDER BY year DESC')
    const locations = await pool.query('SELECT DISTINCT location FROM seasons WHERE location IS NOT NULL ORDER BY location')
    res.json({
...
app.post('/api/seasons', async (req, res) => {
  try {
    const { name, year, location, league_name, organizer, default_game_time } = req.body
    const result = await pool.query(
      'INSERT INTO seasons (name, year, location, league_name, organizer, default_game_time) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [name, year, location ?? null, league_name ?? null, organizer ?? null, default_game_time ?? null]
    )
...
app.patch('/api/seasons/:id', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [name, year, location ?? null, league_name ?? null, organizer ?? null, default_game_time ?? null, req.params.id]
    )
...
app.post('/api/games', async (req, res) => {
  try {
    const { opponent, game_date, game_time, game_type, season_id, notes } = req.body
    const result = await pool.query(
      ...
      [opponent, game_date, game_time, game_type, season_id ?? null, notes ?? null]
    )
    res.json(result.rows[0])
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : String(err) })
...
app.patch('/api/games/:id', async (req, res) => {
  try {
    ...
    if (outcome_override !== undefined) { updates.push(`outcome_override = $${i++}`); params.push(outcome_override) }
    if (gameResult !== undefined) { updates.push(`result = $${i++}`); params.push(gameResult) }
    if (updates.length === 0) return res.json({ success: true })
    params.push(req.params.id)
    await pool.query(`UPDATE games SET ${updates.join(', ')} WHERE id = $${i}`, params)
...
app.delete('/api/games/:id', async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM game_events WHERE game_id = $1', [req.params.id])
    await pool.query('DELETE FROM game_lineups WHERE game_id = $1', [req.params.id])
...
app.delete('/api/games/:id/lineups/:playerId', async (req, res) => {
  try {
    ...
    if (lineup_name) { q += ' AND lineup_name = $3'; params.push(lineup_name) }
...
app.post('/api/events/goal', async (req, res) => {
  try {
    ...
    const gameResult = await pool.query('SELECT game_date FROM games WHERE id = $1', [gameId])
    ...
    const result = await pool.query(
      ...
      tsParam
        ? [gameId, playerId ?? null, relatedPlayerId ?? null, eventType ?? 'Goal', notes ?? null, tsParam]
...
app.delete('/api/events/:id', async (req, res) => {
  try {
    const evt = await pool.query('SELECT game_id FROM game_events WHERE id = $1', [req.params.id])
    const result = await pool.query('DELETE FROM game_events WHERE id = $1 RETURNING *', [req.params.id])
...
app.patch('/api/events/:id', async (req, res) => {
  try {
    ...
    const params: unknown[] = []
    let i = 1
    if (playerId !== undefined) { updates.push(`player_id = $${i++}`); params.push(playerId) }
    if (relatedPlayerId !== undefined) { updates.push(`related_player_id = $${i++}`); params.push(relatedPlayerId) }
    if (notes !== undefined) { updates.push(`notes = $${i++}`); params.push(notes) }
    if (eventType !== undefined) { updates.push(`event_type = $${i++}`); params.push(eventType) }
    if (updates.length === 0) return res.json({ success: true })
    params.push(req.params.id)
    const result = await pool.query(
...
async function updateGameScore(gameId: number | string) {
  ...
  const gameRow = await pool.query('SELECT outcome_override FROM games WHERE id = $1', [gameId])
...
app.get('/api/players/season-roster', async (req, res) => {
  try {
    ...
    if (!seasonId) {
      const all = await pool.query('SELECT id, display_name, gender_match, is_sub, number FROM players ORDER BY display_name')
...
app.patch('/api/players/:id/seasons', async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM season_players WHERE player_id = $1', [req.params.id])
...
app.post('/api/players', async (req, res) => {
  try {
    const { display_name, first_name, last_name, gender_match, phone, number, position, is_sub, season_ids } = req.body
    const result = await pool.query(
      `INSERT INTO players (display_name, first_name, last_name, gender_match, phone, number, position, is_sub)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [display_name, first_name ?? display_name, last_name ?? null, gender_match ?? null, phone ?? null, number ?? null, position ?? null, is_sub ?? false]
    )
...
app.patch('/api/players/:id', async (req, res) => {
  try {
    const { display_name, first_name, last_name, gender_match, phone, number, position } = req.body
    const updates: string[] = []
    const params: unknown[] = []
    let i = 1
    ...
    if (gender_match !== undefined) { updates.push(`gender_match = $${i++}`); params.push(gender_match) }
    if (phone !== undefined) { updates.push(`phone = $${i++}`); params.push(phone) }
    if (number !== undefined) { updates.push(`number = $${i++}`); params.push(number === '' ? null : number) }
...
app.delete('/api/players/:id', async (req, res) => {
  try {
    const playerId = req.params.id
    await pool.query('DELETE FROM game_events WHERE player_id = $1 OR related_player_id = $1', [playerId])
    await pool.query('DELETE FROM season_players WHERE player_id = $1', [playerId])
    await pool.query('DELETE FROM game_lineups WHERE player_id = $1', [playerId])
...
app.delete('/api/chat/history', async (req, res) => {
  try {
    const { session_id } = req.query
    if (session_id) await pool.query('DELETE FROM chat_logs WHERE session_id = $1', [session_id])
    res.json({ success: true })
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : String(err) })
  }
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const GEMINI_API_KEY = process.env.GEMINI_API_KEY || 'AIzaSyDOzMxmNWTenYfPW4eEauzF8P3QVmr0a0g'

    // Fetch DB context for system prompt
    const seasonsData = await pool.query('SELECT id, name, year, organizer FROM seasons ORDER BY year DESC LIMIT 10')
    const gamesData = await pool.query('SELECT id, opponent, game_date, our_score, their_score, result, season_id FROM games ORDER BY game_date DESC LIMIT 20')
    const playersData = await pool.query('SELECT id, display_name, position, gender_match FROM players ORDER BY display_name LIMIT 50')
        ...
        try {
          ...
          if (actionType === 'ADD_GOAL') {
            await fetch(`http://localhost:${PORT}/api/events/goal`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ gameId: data.gameId, playerId: data.playerId ?? null, relatedPlayerId: data.relatedPlayerId ?? null, eventType: data.eventType ?? 'Goal' })
            })
